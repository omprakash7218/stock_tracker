...
def portfolio_service(portfolio_id, current_user, db):
    ...
    if not portfolio:
        return None
    trades = (
        db.query(Trade).join(Asset).filter(Trade.portfolio_id == portfolio_id).all()
    )
    if not trades:
        return {
    ...
    for trade in trades:
        current_price = PriceService.get_price(
            trade.asset.symbol, trade.asset.asset_type
        )
    ...
    portfolio_profit_loss_percentage = (
        (portfolio_profit_loss / total_amount_invested) * 100
